 ...
 * @param <T> Enum Type
 */
public class EnumResourceStringHelper<T extends Enum<T>> extends EnumResourceHelper<T, String> {
	 ...
	 */
	public EnumResourceStringHelper(@NonNull Class<T> enumType, @NonNull Context context, @Nullable String resourcePrefix,
	...
	@Override
	protected void initMap(@NonNull Context context) {
		for (T value : enumType.getEnumConstants()) {
			int resourceId = context.getResources().getIdentifier(getResourceName(value), "string", context.getPackageName());
			if (resourceId != 0) {
				enumMap.put(value, context.getResources().getString(resourceId));
			} else {
				enumMap.put(value, value.name());
				Log.i(LOG_TAG, "No string found with name : \"" + getResourceName(value) + "\" default string use : \"" + value.name() + "\"");
			}
		}
	}
}
